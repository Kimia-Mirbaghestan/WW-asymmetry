...
def getStack(var, samples, excludeSig = False):
    ...
    for s in samples:
        if not os.path.exists( "Histogram_Root/"+ s + "_histos.root"):
            ...
            f = ROOT.TFile("Histogram_Root/" + s + "_histos.root")
            h = f.Get(var)
...
def plotVar(var, samples,isData = False, logScale = False ):
    ...
    if not os.path.exists( "Histogram_Root/" + "ttbar_histos.root"):
        print ("File ttbar_histos.root does not exist. Please, check to have processed the corresponding sample")
    else:            
        f = ROOT.TFile("Histogram_Root/" + "ttbar_histos.root")
        h = f.Get(var)
    ...
    if(isData): 
        ...
        c.SaveAs("Observables/"+ var + ".pdf")
    else: c.SaveAs("Observables/"+ var + "_MC.pdf")
...
def plotVarNorm(var, samples, logScale = False):
    ...
    leg.Draw("SAME")
    c.SaveAs("Observables/"+ var + "_Norm_MC.pdf")
...
def plotShapes(var, samples, logScale = False):
    ...
    if not os.path.exists( "Histogram_Root/" + "ttbar_histos.root"):
        ...
        h.SetLineWidth(2)
        if(h.Integral()!=0.):h.Scale(1.)
        h.Draw("histsame")
        leg.AddEntry(h, "Signal (ttbar)","L")

    leg.Draw("SAME")
    c.SaveAs("Observables/"+ var + "_Shape_MC.pdf")
...
def getHisto(var, sample):
    ...
    h = ROOT.TH1F()
    filename = "Histogram_Root/" + sample + "_histos.root"
